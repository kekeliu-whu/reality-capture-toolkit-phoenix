...
class Rigid3 {
 ...
 public:
  ...
  Rigid3() : translation_(Vector_::Zero()), rotation_(Quaternion_::Identity()) {}
  Rigid3(const Vector_& translation, const Quaternion_& rotation) : translation_(translation), rotation_(rotation) {}
  Rigid3(const Vector7_& vector) : translation_(vector.template block<3, 1>(0, 0)), rotation_(vector.template block<4, 1>(3, 0)) {}

  static Rigid3 Rotation(const Quaternion_& rotation) { return Rigid3(Vector_::Zero(), rotation); }

  static Rigid3 Translation(const Vector_& vector) { return Rigid3(vector, Quaternion_::Identity()); }
  ...
  std::string DebugString() const {
    std::stringstream ss;
    ss << "{ t: [" << translation().x() << ", " << translation().y() << ", " << translation().z() << "], q: [" << rotation().w() << ", "
...
Rigid3<FloatType> operator*(const Rigid3<FloatType>& lhs, const Rigid3<FloatType>& rhs) {
  return Rigid3<FloatType>(lhs.rotation() * rhs.translation() + lhs.translation(), (lhs.rotation() * rhs.rotation()).normalized());
}

template <typename FloatType>
inline typename Rigid3<FloatType>::Vector_ operator*(const Rigid3<FloatType>& rigid, const typename Rigid3<FloatType>::Vector_& point) {
