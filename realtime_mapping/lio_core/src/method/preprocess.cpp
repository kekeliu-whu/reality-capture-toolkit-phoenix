...
void PreProcess::doCorrectImu(const SensorParam::ImuParam &param, std::vector<ImuMsg> &imus)
{
  if (!param.enabled)
  {
    lslog(LSLOG_WARNING) << "Imu instrinsic calibration is disabled";
    return;
  }
  for (auto &imu : imus)
  {
...
void PreProcess::doCorrectLidar(const SensorParam::LidarParam &param, PointCloud::Ptr &cloud)
{
  if (!param.enabled)
  {
    lslog(LSLOG_WARNING) << "Lidar instrinsic calibration is disabled";
    return;
  }
    ...
    PointCloud::Ptr &cloud)
{
  if (!param.enabled)
  {
    lslog(LSLOG_WARNING) << "Motor motion compensation is disabled";
    return;
  }
