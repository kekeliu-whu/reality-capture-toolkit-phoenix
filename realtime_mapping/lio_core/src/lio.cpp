    ...
    : io_utils_(lio_param.extrinsic_param.motor_param.enabled, lio_param.preprocess_param.sweep_cut_auto), lidar_fusion_(lio_param.kf_param.k_for_adaptive_search)
{
  ...
  lslog(LSLOG_INFO) << "init success!";
}

LioCore::~LioCore()
{
  lslog(LSLOG_INFO) << "lio_core exiting ...";
  stop();
  lslog(LSLOG_INFO) << "lio_core exit success";
}
...
void LioCore::stop()
{
  ...
  should_exit_ = true;
  lslog(LSLOG_INFO) << "lio_core exiting from stop() ...";
  ...
  lslog(LSLOG_INFO) << "lio_core exit success from stop()";
...
void LioCore::mappingLoop()
{
  ...
  while (ProcessState::STOP != process_state_ && !should_exit_)
  {
    ...
    lslog(LSLOG_INFO) << "syncPackagesT:" << syncPackagesT.Toc();

    // 0. check
    TicToc totalT;
    lslog(LSLOG_INFO) << std::fixed << "measurement.pcl_end_time:" << measurement.pcl_end_time;
    lslog(LSLOG_INFO) << std::fixed << "measurement.pcl_start_time:" << measurement.pcl_start_time;

    // 1. preprocess
    TicToc preprocessT;
    preprocess_.process(lio_param_, measurement);
    lslog(LSLOG_INFO) << "preprocessT:" << preprocessT.Toc();
    ...
    lslog(LSLOG_INFO) << "predictT:" << predictT.Toc();
    ...
    lslog(LSLOG_INFO) << "undistortT:" << undistortT.Toc();
    ...
    lslog(LSLOG_INFO) << "sweep_id:" << measurement.sweep_id;
    lslog(LSLOG_INFO) << "originSize:" << measurement.lidar_points->size();
    lslog(LSLOG_INFO) << "undistortSize:" << undistort_pcl->size();
    lslog(LSLOG_INFO) << "downMapSize:" << downsample_map_body->size();
    lslog(LSLOG_INFO) << "downSurfSize:" << downsample_surf_body->size();
    lslog(LSLOG_INFO) << "downsampleT:" << downsampleT.Toc();
    ...
    lslog(LSLOG_INFO) << "updateT:" << updateT.Toc();
    ...
    if (need_update_map)
    {
      ...
      lslog(LSLOG_INFO) << "mapT:" << mapT.Toc();
    }
    ...
    lslog(LSLOG_INFO) << "totalT:" << totalT.Toc();
  }
  lslog(LSLOG_INFO) << "exit from mappingLoop";
}
...
void LioCore::publisherLoop()
{
  ...
  while (1)  // todo control publisher loop
  {
    ...
    else
    {
      ...
      if (ProcessState::STOP == process_state_ && should_exit_)
      {
        lslog(LSLOG_INFO) << "exit from publisherLoop";
        break;
      }
    }
