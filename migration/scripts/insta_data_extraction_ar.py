...
INSV_NUM_STREAMS = 2  # INSV 内部摄像头流数量 (cam0, cam1)
CAM2_NUM_STREAMS = 1  # MOV 对应单目输出 (cam2)
CAM2_START_INDEX = 2
TOTAL_CAMERAS = INSV_NUM_STREAMS + CAM2_NUM_STREAMS
...
def extract_frames_from_video(
    ...
    参数:
        ...
        start_cam_index: 输出摄像头编号起始值
        frame_sample_rate: 采样率，每隔N帧取1帧 (1表示取所有帧, 2表示每2帧取1帧)
        all_timestamps: 所有原始帧的时间戳列表，用于按采样率生成输出时间戳
        rename_timestamps: 直接用于输出命名的时间戳列表，优先级高于 all_timestamps
    ...
    # 创建输出目录，并清空旧的帧文件
    sampled_timestamps = list(rename_timestamps) if rename_timestamps is not None else []

    for i in range(num_streams):
        cam_idx = start_cam_index + i
        cam_dir = os.path.join(output_base_dir, f"cam{cam_idx}")
    ...
    for stream_idx in range(num_streams):
        cam_idx = start_cam_index + stream_idx
        cam_dir = os.path.join(output_base_dir, f"cam{cam_idx}")

        print(f"[OK] Processing cam{cam_idx} from stream {stream_idx}...")
        ...
        try:
            ...
            print(f"  [OK] Extracted {len(temp_frames)} frames for cam{cam_idx}")

            # 根据采样率进行采样并重命名文件
            sampled_images = []
            current_sampled_timestamps = []
            sampled_frame_indexes = list(range(0, len(temp_frames), frame_sample_rate))

            if rename_timestamps is not None:
                timestamp_sequence = list(rename_timestamps)
            elif all_timestamps:
                timestamp_sequence = [
                    all_timestamps[idx]
                    for idx in sampled_frame_indexes
                    if idx < len(all_timestamps)
                ]
                sampled_timestamps = timestamp_sequence
            else:
                timestamp_sequence = []

            if not timestamp_sequence:
                print(f"  [WARN] No timestamps available for cam{cam_idx}, skipping frame export")
                continue

            if len(timestamp_sequence) != len(sampled_frame_indexes):
                print(
                    f"  [WARN] cam{cam_idx} sampled frame count ({len(sampled_frame_indexes)}) and timestamp count ({len(timestamp_sequence)}) differ; using {min(len(sampled_frame_indexes), len(timestamp_sequence))} aligned frames"
                )

            for sample_idx, frame_idx in enumerate(sampled_frame_indexes):
                if sample_idx >= len(timestamp_sequence):
                    break
                ...
                # 确定新文件名
                timestamp = timestamp_sequence[sample_idx]
                ...
                try:
                    ...
                    current_sampled_timestamps.append(timestamp)
                except Exception as e:
            ...
            print(
                f"  [OK] cam{cam_idx} sampling complete: {len(sampled_images)} frames (sample rate: {frame_sample_rate})"
            )

            # 保存图片列表
            image_lists[f"cam{cam_idx}"] = sampled_images

            if rename_timestamps is not None and len(current_sampled_timestamps) != len(rename_timestamps):
                print(
                    f"  [WARN] cam{cam_idx} exported {len(current_sampled_timestamps)} frames, expected {len(rename_timestamps)} from reference timestamps"
                )

        except subprocess.CalledProcessError as e:
            print(f"[ERROR] Failed to extract frames for cam{cam_idx}")
