...
def interpolate_pose(timestamp, imu_poses):
    ...
    # SLERP for rotation
    q_before = Rotation.from_quat([before['rx'], before['ry'], before['rz'], before['rw']])
...
def process_poses(poses_file, calib_file, image_folder, output_file, image_list=None):
    ...
    print(f"✓ Found {len(camera_folders)} camera folder(s): {sorted(camera_folders.keys())}")
    ...
    with open(output_file, 'w', newline='') as f:
        ...
        # Write header
        writer.writerow(['image', 'x', 'y', 'z', 'roll', 'pitch', 'yaw', 
        ...
        for cam_idx in sorted(camera_folders.keys()):
            ...
            for img_path in image_files:
                ...
                imu_pose = interpolate_pose(timestamp, imu_poses)
                if imu_pose is None:
                    continue
                ...
                writer.writerow([
                    ...
                    cam_pose['tx'], cam_pose['ty'], cam_pose['tz'],
                    0.0, 0.0, 0.0,
                    cam_pose['rx'], cam_pose['ry'], cam_pose['rz'], cam_pose['rw'],
                    timestamp
                ])
