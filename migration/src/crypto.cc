...
void EncryptedLogSink::send(google::LogSeverity severity, const char* full_filename, const char* base_filename, int line, const struct ::tm* tm_time,
                            const char* message, size_t message_len) {
  std::string msg(message, message_len);
  std::string encrypted = EncryptAES(msg);
  spdlog::info("[EncryptedLog] {}", encrypted);
}
