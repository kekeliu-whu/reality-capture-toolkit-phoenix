...
std::vector<TimeStampedState> ReadLidarStates(const std::string &filename) {
  ...
  while (infile >> state.time >> state.pos[0] >> state.pos[1] >> state.pos[2] >> state.rot.x() >> state.rot.y() >>
         state.rot.z() >> state.rot.w()) {
    state.rot.normalize();
...
// get T_cw
std::deque<ImageInfo> ComputeTriggerCameraPoses(const std::vector<TimeStampedState> &states,
                                                const std::vector<double>           &trigger_timestamps) {
  // cam0
  Eigen::Quaterniond rot1{0.55503322346987594, -0.43084031432705472, -0.42751705014452018, -0.5688092089892105};
  Eigen::Vector3d    pos1{0.0015287756742656672, 0.11306746601651824, -0.00036282575151124277};

  // lidar
  Eigen::Quaterniond rot2{0.89572341110116316, 0.00093473423644999923, -0.00016557714642211738, 0.4446106944973463};
  Eigen::Vector3d    pos2{-0.00079115205805683781, 0.00011542889283456398, 0.048006061318188097};
  ...
  for (auto &t : trigger_timestamps) {
    auto it = std::lower_bound(states.begin(), states.end(), t,
                               [](const TimeStampedState &lhs, double rhs) { return lhs.time < rhs; });
    --it;
    ...
    if (it == states.begin() - 1 || it == states.end() - 1) {
      ...
      if (s < 0) {
        spdlog::error("CHECK_GE failed: s >= 0");
        exit(1);
      }
      if (s > 1) {
        spdlog::error("CHECK_LE failed: s <= 1");
        exit(1);
      }
...
int main() {
  ...
  auto camera_trigger_places = ReadTriggerTimestamps(
      "/home/rick/Documents/raw_data/navvis/vlx/2022-07-28_03.43.39/colorize-kk/trigger_list.txt");
  ...
  for (int i = 500; i < lidar_states.size(); ++i) {
    ...
    for (auto &p : scan) {
      pcl::PointXYZRGB pt;
      pt.getVector3fMap() =
          (lidar_states[i].rot * p.getVector3fMap().cast<double>() + lidar_states[i].pos).cast<float>();
  ...
  //   auto filename =
  //   (boost::format("/home/rick/Documents/raw_data/navvis/vlx/2022-07-28_03.43.39/colorize-kk/cam-export/%05d-cam0.jpg")
  //   % i).str(); auto img      = cv::imread(filename);
  ...
  //   Eigen::Vector3d p_c = K * (camera_states[i].state.rot * p.getVector3fMap().cast<double>() +
  //   camera_states[i].state.pos); if (p_c[2] <= 0) {
  //     continue;
  //   }
  ...
  while (!scans.empty() && !camera_states.empty()) {
    ...
    if (camera_time < lidar_time) {
      ...
      auto filename =
          (boost::format(
               "/home/rick/Documents/raw_data/navvis/vlx/2022-07-28_03.43.39/colorize-kk/cam-export/%05d-cam0.jpg") %
           img_id)
              .str();
      ...
      img_ptr->m_cam_K << 910.6, 0, 2732, 0, 910.6, 1820, 0, 0, 1;
