...
void InsertColorCandidate(PointCandidateState& state,
                          CandidateBlockPool& candidate_pool,
                          int candidate_limit,
                          float distance,
  ...
  const PackedColorCandidate packed = {distance, PackColor(color)};
  ...
  const int new_count = std::min<int>(state.count + 1, candidate_limit);
...
std::optional<Eigen::Vector2d> ProjectDepthPixel(
    const colmap::Camera& depth_camera,
    const Eigen::Vector3d& pt_in_cam) {
  if (pt_in_cam.z() <= 0.0) {
    return std::nullopt;
  }

  const auto pixel = depth_camera.ImgFromCam(pt_in_cam / pt_in_cam.z());
  if (!pixel.has_value() || pixel->x() < 0.0 ||
      pixel->x() >= static_cast<double>(depth_camera.width) ||
      pixel->y() < 0.0 ||
      pixel->y() >= static_cast<double>(depth_camera.height)) {
    return std::nullopt;
  }
...
bool IsDepthVisible(const colmap::Camera& depth_camera,
                    const Eigen::Vector3d& pt_in_cam,
                    const cv::Mat& depth_meters) {
  const auto depth_pixel_opt = ProjectDepthPixel(depth_camera, pt_in_cam);
...
colmap::Camera ScaleCameraToSize(const colmap::Camera& camera,
                                 int width,
                                 int height) {
  if (camera.width == 0 || camera.height == 0 || width <= 0 || height <= 0) {
    throw std::invalid_argument("Depth camera sizes must be positive");
  }
  colmap::Camera scaled = camera;
  scaled.Rescale(static_cast<size_t>(width), static_cast<size_t>(height));
  return scaled;
}

xsfm_post::OpenCVFisheyeIntrinsics ToFisheyeIntrinsics(
    const colmap::Camera& camera) {
  if (camera.ModelName() != "OPENCV_FISHEYE" || camera.params.size() != 8) {
    throw std::invalid_argument(
        "Generated fisheye depth requires an OPENCV_FISHEYE camera");
  }
  return {static_cast<int>(camera.width),
...
std::array<double, 9> RotationArray(const colmap::Rigid3d& pose) {
  const Eigen::Matrix3d rotation = pose.rotation().toRotationMatrix();
  std::array<double, 9> result{};
  for (int row = 0; row < 3; ++row) {
    for (int col = 0; col < 3; ++col) {
      result[static_cast<size_t>(row) * 3 + col] = rotation(row, col);
    }
  }
  return result;
}

std::array<double, 3> TranslationArray(const colmap::Rigid3d& pose) {
  return {pose.translation().x(),
          pose.translation().y(),
          pose.translation().z()};
}

void WriteGeneratedDepth(const std::filesystem::path& filename,
                         const cv::Mat& depth_meters) {
  std::filesystem::create_directories(filename.parent_path());
  cv::Mat bgra(depth_meters.rows,
               depth_meters.cols,
               CV_8UC4,
               cv::Scalar(0, 0, 0, 0));
  for (int y = 0; y < depth_meters.rows; ++y) {
    for (int x = 0; x < depth_meters.cols; ++x) {
      ...
      const uint32_t millimeters = static_cast<uint32_t>(
          std::llround(static_cast<double>(depth) * 1000.0));
      cv::Vec4b& pixel = bgra.at<cv::Vec4b>(y, x);
      pixel[0]         = static_cast<uint8_t>((millimeters >> 16) & 0xff);
      pixel[1]         = static_cast<uint8_t>((millimeters >> 8) & 0xff);
      pixel[2]         = static_cast<uint8_t>(millimeters & 0xff);
      pixel[3]         = static_cast<uint8_t>((millimeters >> 24) & 0xff);
    }
  }
  if (!cv::imwrite(filename.string(), bgra)) {
    throw std::runtime_error("Failed to write generated depth image: " +
                             filename.string());
  }
}
...
std::optional<std::pair<double, cv::Vec3b>> ComputeColorCandidates(
  ...
  if (!IsDepthVisible(depth_camera, pt_in_cam, depth_meters)) {
    return std::nullopt;
  }
...
void WritePointCloudToLAS(
    ...
    const std::vector<uint8_t>& view_counts) {
...
void PerformXColor(const std::vector<Image>& images,
  ...
  std::unique_ptr<xsfm_post::CudaDepthRenderer> fisheye_depth_renderer;
  if (fisheye_depth_options.generate) {
    if (!xsfm_post::HasCudaDevice()) {
      throw std::runtime_error(
          "Generated fisheye depth requires an available CUDA device");
    }
    if (!(fisheye_depth_options.scale > 0.0) ||
        fisheye_depth_options.voxel_size <= 0.0f ||
        fisheye_depth_options.visibility_tolerance < 0.0f ||
        fisheye_depth_options.gpu_chunk_points <= 0) {
      throw std::invalid_argument("Invalid generated fisheye depth options");
    }
    for (const Image& image : images) {
      if (image.camera.ModelName() != "OPENCV_FISHEYE") {
        throw std::invalid_argument(
            "All cameras must use OPENCV_FISHEYE when generating fisheye depth; "
            "got " +
            image.camera.ModelName() + " for " + image.filename);
      }
    }
    spdlog::info("Uploading {} points once for direct fisheye depth rendering",
                 cloud_rgb.size());
    std::vector<xsfm_post::DepthWorldPoint> depth_points;
    std::vector<xsfm_post::DepthWorldNormal> depth_normals;
    depth_points.reserve(cloud_rgb.size());
    if (fisheye_depth_options.gpu_visibility) {
      depth_normals.reserve(cloud_rgb.size());
    }
    for (const auto& point : cloud_rgb) {
      // Preserve the original point index. Non-finite coordinates are rejected
      // by the CUDA projection kernel, just as they are by the CPU path.
      depth_points.push_back({point.x, point.y, point.z});
      if (fisheye_depth_options.gpu_visibility) {
        depth_normals.push_back(
            {point.normal_x, point.normal_y, point.normal_z});
      }
    }
    if (fisheye_depth_options.gpu_visibility) {
      fisheye_depth_renderer = std::make_unique<xsfm_post::CudaDepthRenderer>(
          depth_points, depth_normals);
    } else {
      fisheye_depth_renderer =
          std::make_unique<xsfm_post::CudaDepthRenderer>(depth_points);
    }
    depth_points.clear();
    depth_points.shrink_to_fit();
    depth_normals.clear();
    depth_normals.shrink_to_fit();
    spdlog::info(
        "Direct fisheye depth enabled: scale={}, voxel={} m, max_distance={} m, visibility_tolerance={} m",
        fisheye_depth_options.scale,
        fisheye_depth_options.voxel_size,
        fisheye_depth_options.max_distance,
        fisheye_depth_options.visibility_tolerance);
  }

  const bool use_gpu_color_fusion =
      fisheye_depth_options.generate &&
      fisheye_depth_options.gpu_visibility &&
      fisheye_depth_options.gpu_color_fusion;
  if (use_gpu_color_fusion) {
    fisheye_depth_renderer->ResetFusedColors();
    spdlog::info(
        "CUDA NVS-inspired color selection enabled: Lixel-style geometry "
        "weighting, spatially coherent weighted multi-view consensus, {} "
        "fusion, and log-luminance exposure compensation stay on the GPU",
        fisheye_depth_options.smooth_fusion ? "robust-mean" : "sharp-view");
  } else if (fisheye_depth_options.gpu_color_fusion) {
    spdlog::info(
        "CUDA color fusion requires generated fisheye depth and GPU "
        "visibility; using the CPU candidate path");
  }

  // The CPU fallback keeps a compact 8-byte state per point and commits
  // top-k candidate blocks lazily. The GPU fusion path does not allocate this
  // multi-gigabyte candidate pool.
  std::vector<PointCandidateState> point_candidate_states;
  std::unique_ptr<CandidateBlockPool> candidate_pool;
  if (!use_gpu_color_fusion) {
    point_candidate_states.resize(cloud_rgb.size());
    candidate_pool = std::make_unique<CandidateBlockPool>(cloud_rgb.size());
  }

  // Step 2: 逐图片处理，实时读取深度图并对整片点云做深度可见性过滤
  spdlog::info("Collecting color candidates from images...");
  std::unordered_map<std::string, cv::Mat> mask_cache;
  std::string uploaded_visibility_mask;
  std::future<cv::Mat> prefetched_image;
  for (int image_idx = 0; image_idx < images.size(); ++image_idx) {
    ...
    if (!fisheye_depth_options.generate && !image.has_depth) {
      spdlog::warn("Skipping image without depth image: {}", image.filename);
      print_image_progress();
      continue;
    }

    cv::Mat img_cv = image_idx == 0
                         ? cv::imread(image.filename)
                         : prefetched_image.get();
    if (image_idx + 1 < images.size()) {
      const std::string next_filename = images[image_idx + 1].filename;
      prefetched_image                = std::async(std::launch::async, [next_filename]() {
        return cv::imread(next_filename);
      });
    }
    if (img_cv.empty()) {
    ...
    cv::Mat mask;
    if (image.has_mask) {
      const auto mask_iter = mask_cache.find(image.mask_filename);
      if (mask_iter != mask_cache.end()) {
        mask = mask_iter->second;
      } else {
        mask = LoadMask(image);
        mask_cache.emplace(image.mask_filename, mask);
      }
    }
    if (fisheye_depth_options.generate &&
        fisheye_depth_options.gpu_visibility &&
        image.mask_filename != uploaded_visibility_mask) {
      if (mask.empty()) {
        fisheye_depth_renderer->ClearVisibilityMask();
      } else {
        fisheye_depth_renderer->SetVisibilityMask(
            mask.data, mask.cols, mask.rows, mask.step);
      }
      uploaded_visibility_mask = image.mask_filename;
    }
    cv::Mat depth_meters;
    colmap::Camera depth_camera;
    std::optional<xsfm_post::DepthRenderResult> generated_depth;
    std::optional<xsfm_post::FisheyeVisibilityResult> gpu_visibility;
    std::optional<xsfm_post::FisheyeColoringResult> gpu_coloring;
    if (fisheye_depth_options.generate) {
      const int depth_width = std::max(
          1, static_cast<int>(std::lround(image.camera.width *
                                          fisheye_depth_options.scale)));
      const int depth_height = std::max(
          1, static_cast<int>(std::lround(image.camera.height *
                                          fisheye_depth_options.scale)));
      depth_camera = ScaleCameraToSize(image.camera, depth_width, depth_height);
      spdlog::info("Rendering fisheye depth {} / {} at {}x{}",
                   image_idx + 1,
                   images.size(),
                   depth_width,
                   depth_height);
      if (use_gpu_color_fusion) {
        gpu_coloring = fisheye_depth_renderer->RenderFisheyeColor(
            RotationArray(image.pose),
            TranslationArray(image.pose),
            ToFisheyeIntrinsics(depth_camera),
            img_cv.data,
            img_cv.cols,
            img_cv.rows,
            img_cv.step,
            fisheye_depth_options.voxel_size,
            fisheye_depth_options.gpu_chunk_points,
            fisheye_depth_options.max_distance,
            fisheye_depth_options.visibility_tolerance,
            candidate_limit,
            image_idx,
            !fisheye_depth_options.output_path.empty());
        if (!gpu_coloring->depth.empty()) {
          depth_meters = cv::Mat(gpu_coloring->height,
                                 gpu_coloring->width,
                                 CV_32FC1,
                                 gpu_coloring->depth.data());
        }
      } else if (fisheye_depth_options.gpu_visibility) {
        gpu_visibility = fisheye_depth_renderer->RenderFisheyeVisibility(
            RotationArray(image.pose),
            TranslationArray(image.pose),
            ToFisheyeIntrinsics(depth_camera),
            img_cv.cols,
            ...
            fisheye_depth_options.visibility_tolerance,
            !fisheye_depth_options.output_path.empty());
        if (!gpu_visibility->depth.empty()) {
          depth_meters = cv::Mat(gpu_visibility->height,
                                 gpu_visibility->width,
                                 CV_32FC1,
                                 gpu_visibility->depth.data());
        }
      } else {
        generated_depth = fisheye_depth_renderer->RenderFisheye(
            RotationArray(image.pose),
            TranslationArray(image.pose),
            ToFisheyeIntrinsics(depth_camera),
            fisheye_depth_options.voxel_size,
            ...
            fisheye_depth_options.max_distance,
            false);
        depth_meters = cv::Mat(generated_depth->height,
                               generated_depth->width,
                               CV_32FC1,
                               generated_depth->depth.data());
      }
      if (!fisheye_depth_options.output_path.empty()) {
        std::filesystem::path relative_depth = image.name;
        relative_depth.replace_extension(".png");
        WriteGeneratedDepth(
            std::filesystem::path(fisheye_depth_options.output_path) /
                relative_depth,
            depth_meters);
      }
    } else {
      ...
      depth_meters = depth_meters_opt.value();
      depth_camera = ScaleCameraToSize(
          image.depth_camera, depth_meters.cols, depth_meters.rows);
      if (depth_camera.width != image.depth_camera.width ||
          depth_camera.height != image.depth_camera.height) {
        spdlog::info("Scaled depth camera for {} from {}x{} to {}x{}",
                     image.filename,
                     image.depth_camera.width,
                     image.depth_camera.height,
                     depth_camera.width,
                     depth_camera.height);
      }
    }
    ...
    if (gpu_coloring.has_value()) {
      if ((image_idx + 1) % 100 == 0 || image_idx + 1 == images.size()) {
        spdlog::info("Fused {} / {} images entirely on CUDA",
                     image_idx + 1,
                     images.size());
      }
    } else if (gpu_visibility.has_value()) {
      const auto& visible_points = gpu_visibility->visible_points;
      spdlog::info("Coloring image {} / {} from {} CUDA-visible points",
                   image_idx,
                   images.size(),
                   visible_points.size());
      const int visible_count = static_cast<int>(visible_points.size());
#pragma omp parallel for
      for (int visible_idx = 0; visible_idx < visible_count; ++visible_idx) {
        const auto& visible = visible_points[visible_idx];
        const int point_idx = static_cast<int>(visible.point_index);
        if (point_idx < 0 || point_idx >= point_count) {
          continue;
        }
        const int pixel_x = visible.pixel_x;
        const int pixel_y = visible.pixel_y;
        if (pixel_x < 0 || pixel_x >= img_cv.cols || pixel_y < 0 ||
            pixel_y >= img_cv.rows) {
          continue;
        }
        InsertColorCandidate(point_candidate_states[point_idx],
                             *candidate_pool,
                             candidate_limit,
                             visible.distance,
                             img_cv.at<cv::Vec3b>(pixel_y, pixel_x));
      }
    } else {
...
#pragma omp parallel for
    ...
    if (!use_gpu_color_fusion &&
        ((image_idx + 1) % 100 == 0 || image_idx + 1 == images.size())) {
  ...
  PrintProgress(96.0);
  std::vector<uint8_t> output_view_counts(cloud_rgb.size(), 0);
  size_t colored_point_count = 0;
  if (use_gpu_color_fusion) {
    const std::vector<xsfm_post::FusedPointColor> fused_colors =
        fisheye_depth_renderer->DownloadFusedColors(
            fisheye_depth_options.smooth_fusion);
    if (fused_colors.size() != cloud_rgb.size()) {
      throw std::runtime_error("CUDA fused-color result size mismatch");
    }
#pragma omp parallel for reduction(+ : colored_point_count)
    for (int point_idx = 0; point_idx < cloud_rgb.size(); ++point_idx) {
      const auto& fused = fused_colors[point_idx];
      if (fused.view_count == 0) {
        continue;
      }
      cloud_rgb[point_idx].b        = fused.blue;
      cloud_rgb[point_idx].g        = fused.green;
      cloud_rgb[point_idx].r        = fused.red;
      output_view_counts[point_idx] = fused.view_count;
      ++colored_point_count;
    }
  } else {
#pragma omp parallel for
    for (int point_idx = 0; point_idx < cloud_rgb.size(); ++point_idx) {
      ...
      output_view_counts[point_idx] = state.count;
    }
    colored_point_count = candidate_pool->size();
  }
