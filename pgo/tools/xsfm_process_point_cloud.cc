...
DEFINE_string(las_filename,
              "E:\\Users\\rick\\Downloads\\rtk_all.las",
              "Input project path");
DEFINE_string(initial_pose_filename,
              "E:\\Users\\rick\\Downloads\\ImgPose_all.txt",
              "Initial pose filename");
DEFINE_string(output_dir,
              "E:\\Users\\rick\\Downloads\\output",
              "Output directory");
DEFINE_bool(output_full, false, "Output full point cloud, otherwise downsample");
DEFINE_bool(output_normals_pcd,
            true,
            "Whether to output a PCD file with normals estimated, named localenu_normal.pcd. If false, will output localenu.pcd without normals.");
...
class LocalENUTransformer {
 public:
  LocalENUTransformer(const Eigen::Vector2d& offset,
    ...
    oss << std::fixed << std::setprecision(9)
        << "+proj=tmerc +lat_0=" << origin_.y() << " +lon_0=" << origin_.x()
        << " +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs";
    std::string target_enu_proj_str = oss.str();
    transformer_ = proj_create_crs_to_crs(
        ctx_, proj_str_.c_str(), target_enu_proj_str.c_str(), nullptr);
    if (!transformer_) {
      spdlog::info("Failed to create transformer with proj string: {} to {}",
                   proj_str_,
                   target_enu_proj_str);
      exit(1);
    }
  ...
  double GetMeridianConvergence() const {
    ...
    spdlog::info("Meridian convergence at lon: {}, lat: {} is {}",
                 lon,
                 lat,
                 factors.meridian_convergence * 180.0 / M_PI);
    spdlog::info("Mercator scale at lon: {}, lat: {} is {}, {}",
                 lon,
                 lat,
 ...
 private:
  Eigen::Vector2d ComputeOriginLonLat(PJ_CONTEXT* ctx,
                                      const Eigen::Vector2d& offset,
                                      const std::string& proj_str) const {
    PJ* transformer =
        proj_create_crs_to_crs(ctx, proj_str.c_str(), "EPSG:4326", nullptr);
    if (!transformer) {
      spdlog::info(
          "Failed to create transformer with proj string: {} to EPSG:4326",
          proj_str);
      exit(1);
    }
...
void DownsamplePointCloudInternal(const pcl::PointCloud<PointType>& cloud_in,
  ...
  for (auto iter = feat_map.begin(); iter != feat_map.end(); ++iter) {
    cloud_out[i].getVector3fMap() =
        iter->second.center.cast<float>() / iter->second.count;
    i++;
  }
}

void SaveLasOffset(const std::string& filename,
                   double offset_x,
                   double offset_y,
                   double lon,
                   double lat) {
  ...
  ofs << std::fixed << std::setprecision(9) << offset_x << "," << offset_y
      << "," << lon << "," << lat << std::endl;
}

void SaveLasOffsetJson(const std::string& filename,
                       Eigen::Vector2d& offset,
                       const std::string& proj4_str,
                       const Eigen::Vector2d& origin_lonlat) {
  nlohmann::json j;
  j["offset_x"] = offset.x();
  j["offset_y"] = offset.y();
...
void LoadLAS(const std::string& filename,
  ...
  pdal::Options opts;
  opts.add(pdal::Option("filename", colmap::PlatformToUTF8(filename)));
  ...
  if (proj_str.empty()) {
    spdlog::info("Failed to get proj4 string from LAS file metadata.");
    proj_str =
        "+proj=tmerc +lat_0=0 +lon_0=0 +x_0=0 +y_0=0 +datum=WGS84 +units=m "
        "+no_defs";
  }

  spdlog::info("LAS offset: X={:.9f}, Y={:.9f}, proj4: {}",
               offset.x(),
               offset.y(),
               proj_str);
...
void SavePointCloud(const std::string& filename,
  ...
  if (cloud.size() != normals.size()) {
    spdlog::error("Point count ({}) does not match normal count ({})",
                  cloud.size(),
                  normals.size());
    return;
  }
...
void PcaEstimateNormalNoDirect(
  ...
  DownsamplePointCloudInternal<pcl::PointXYZI>(
      *cloud, *cloud_downsampled, downsample_voxel_size);
  const int effective_k =
      std::min<int>(k, static_cast<int>(cloud_downsampled->size()));
  if (effective_k < 3) {
    spdlog::warn("Not enough points to estimate normals.");
    return;
  }
...
#pragma omp parallel for
  for (int i = 0; i < cloud->size(); ++i) {
    std::vector<int> k_indices(effective_k);
    std::vector<float> k_sqr_distances(effective_k);

    if (tree->nearestKSearch(
            cloud->at(i), effective_k, k_indices, k_sqr_distances) <= 0) {
      continue;
    }
    ...
    for (int j : k_indices) {
      Eigen::Vector3d neighbor =
          cloud_downsampled->points[j].getVector3fMap().cast<double>();
...
#pragma omp parallel for
  for (int i = 0; i < cloud->size(); ++i) {
    ...
    if (tree->radiusSearch(cloud->at(i),
                           max_search_radius,
                           k_indices,
                           k_sqr_distances,
                           kNearestNeighbors) <= 0) {
    ...
    for (int j : k_indices) {
      ...
      double w = std::exp(-d_d * d_d / (2 * sigma_d * sigma_d) -
                          d_n * d_n / (2 * sigma_n * sigma_n));
      delta_p += w * d_n;
      sum_w += w;
    }
...
pcl::PointCloud<pcl::PointXYZI>::Ptr TransformToLocalENU(
    const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud_nooffset,
  ...
  for (int i = 0; i < cloud_nooffset->size(); ++i) {
    Eigen::Vector2d localEN = transformer.Transform(
        cloud_nooffset->points[i].getVector2fMap().cast<double>() +
        transformer.getOffset());
...
void SaveToLocalENU(const std::string& filename,
                    const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud_nooffset,
                    const LocalENUTransformer& transformer) {
  spdlog::info(
      "Transforming to local ENU coordinates, origin (lat, lon): {}, {}",
      transformer.getOriginLonLat().x(),
      transformer.getOriginLonLat().y());
  pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_out =
      TransformToLocalENU(cloud_nooffset, transformer);
...
void SaveToLocalENU(const std::string& filename,
                    const std::string& initial_pose_filename,
                    const Eigen::Vector2d& offset,
                    const LocalENUTransformer& transformer) {
  std::ifstream file(initial_pose_filename);
  if (!file) {
    spdlog::error("{}", initial_pose_filename);
    exit(1);
  }
  ...
  auto delta_rot =
      Eigen::AngleAxisd(-transformer.GetMeridianConvergence() / 180.0 * M_PI,
                        Eigen::Vector3d(0, 0, 1));

  std::string file_path;
  double tx, ty, tz, roll, pitch, yaw, qx, qy, qz, qw, timestamp;
  while (file >> file_path >> tx >> ty >> tz >> roll >> pitch >> yaw >> qx >>
         qy >> qz >> qw >> timestamp) {
    Eigen::Vector3d pos(tx - offset.x(), ty - offset.y(), tz);
    Eigen::Quaterniond rot(qw, qx, qy, qz);

    Eigen::Vector2d transformed =
        transformer.Transform(Eigen::Vector2d(tx, ty));

    // Eigen::Vector3d pos_enu    = delta_rot * pos;
    Eigen::Quaterniond rot_enu = delta_rot * rot;

    outfile << std::fixed << std::setprecision(9) << file_path << " "
            << transformed.x() << " " << transformed.y() << " " << tz << " "
            << 0 << " " << 0 << " " << 0 << " " << rot_enu.x() << " "
            << rot_enu.y() << " " << rot_enu.z() << " " << rot_enu.w() << " "
            << timestamp << std::endl;
  }
}
...
int main(int argc, char** argv) {
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_nooffset(
      new pcl::PointCloud<pcl::PointXYZI>);

  if (!boost::filesystem::is_regular_file(FLAGS_las_filename)) {
    spdlog::info("FLAGS_las_filename is not a regular file");
    exit(1);
  }
  ...
  if (!FLAGS_output_full) {
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_downsampled(
        new pcl::PointCloud<pcl::PointXYZI>);
    DownsamplePointCloudInternal<pcl::PointXYZI>(
        *cloud_nooffset, *cloud_downsampled, kDownsampleVoxelSize);
  ...
  spdlog::info("Meridian convergence at origin: {} degrees.",
               enu_transformer.GetMeridianConvergence());

  spdlog::info("Save to {}", FLAGS_las_filename + "_offset.json");
  SaveLasOffsetJson(FLAGS_output_dir + "/localenu.json",
                    offset,
                    proj_str,
                    enu_transformer.getOriginLonLat());

  if (FLAGS_output_normals_pcd) {
    spdlog::info("Estimating normals for local ENU point cloud...");
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_localenu =
        TransformToLocalENU(cloud_nooffset, enu_transformer);
    std::vector<Eigen::Vector3f> normals;
    PcaEstimateNormalNoDirect(
        cloud_localenu, kNearestNeighbors, kDownsampleVoxelSize, normals);
    SavePointCloud(
        FLAGS_output_dir + "/localenu_normal.pcd", *cloud_localenu, normals);
  } else {
    SaveToLocalENU(
        FLAGS_output_dir + "/localenu.pcd", cloud_nooffset, enu_transformer);
  }

  SaveToLocalENU(FLAGS_output_dir + "/localenu_pose.txt",
                 FLAGS_initial_pose_filename,
                 offset,
                 enu_transformer);
