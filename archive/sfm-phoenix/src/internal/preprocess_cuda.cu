...
__global__ void rgb_hwc_to_rgb_chw_kernel(const unsigned char* __restrict__ src,
                                           float* __restrict__ dst,
                                           int H, int W) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int hw = H * W;
...
void rgb_hwc_to_rgb_chw_gpu(const unsigned char* src_rgb_hwc,
    ...
    int threads = 256;
    int blocks = (hw + threads - 1) / threads;
    rgb_hwc_to_rgb_chw_kernel<<<blocks, threads, 0, stream>>>(
        src_rgb_hwc, dst_chw_gpu, H, W);
}
