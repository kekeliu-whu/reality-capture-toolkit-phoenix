...
INDEX = "/tmp/navvis_vendor_filter_stages.jsonl"
MAX_HITS = 30
open(INDEX, "w").close()
...
class StageFinish(gdb.FinishBreakpoint):
    ...
    def stop(self):
        path = "/tmp/navvis_vendor_filter_stage_%02d_out.bin" % self.hit
        ...
        gdb.write("filter stage %d owner=%#x %d -> %d\n" %
                  (self.hit, self.owner, self.input_count, output_count))
        return False

class StageBreakpoint(gdb.Breakpoint):
    def __init__(self):
    ...
    def stop(self):
        ...
        path = "/tmp/navvis_vendor_filter_stage_%02d_in.bin" % self.hits
        input_count = save_cloud(path, input_cloud)
        append({"hit": self.hits, "owner": owner,
                "input_count": input_count, "input": path})
        StageFinish(self.hits, owner, output_cloud, input_count)
        return False
