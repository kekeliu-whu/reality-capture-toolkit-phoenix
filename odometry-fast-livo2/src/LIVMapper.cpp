...
{    
  ...
  if (pose_output_en) 
  {
    ...
    if (!pos_opend) 
    {
      ...
      pos_opend = true;
      if (!evoFile.is_open()) spdlog::error("open fail\n");
    } 
    else 
    {
...
void LIVMapper::img_cbk(const sensor_msgs::CompressedImageConstPtr &msg_in)
{
  if (!img_en) return;
  cv_bridge::CvImagePtr cv_ptr;
  cv_ptr = cv_bridge::toCvCopy(msg_in, sensor_msgs::image_encodings::BGR8);
  sensor_msgs::Image::Ptr msg = cv_ptr->toImageMsg();

  // sensor_msgs::Image::Ptr msg(new sensor_msgs::Image(*msg_in));

  ROS_INFO("Received image at time: %.6f", msg->header.stamp.toSec());
