...
& "C:\Program Files\TensorRT-10.16.1.11\bin\trtexec.exe" `
  --onnx="d:\codes\reality-capture-toolkit\feature_extraction\models\aliked_backbone.onnx" `
  --saveEngine="d:\codes\reality-capture-toolkit\feature_extraction\engines\aliked_backbone.engine" `
...
& "C:\Program Files\TensorRT-10.16.1.11\bin\trtexec.exe" `
  --onnx="d:\codes\reality-capture-toolkit\feature_extraction\models\aliked_sddh.onnx" `
  --saveEngine="d:\codes\reality-capture-toolkit\feature_extraction\engines\aliked_sddh.engine" `
...
& "C:\Program Files\TensorRT-10.16.1.11\bin\trtexec.exe" `
  --onnx="d:\codes\reality-capture-toolkit\feature_extraction\models\lightglue.onnx" `
  --saveEngine="d:\codes\reality-capture-toolkit\feature_extraction\engines\lightglue.engine" `
...
```powershell
"D:\Program Files\Microsoft Visual Studio\2022\Enterprise\Common7\IDE\CommonExtensions\Microsoft\CMake\CMake\bin\cmake.exe" -B feature_extraction/build -S feature_extraction `
...
"D:\Program Files\Microsoft Visual Studio\2022\Enterprise\Common7\IDE\CommonExtensions\Microsoft\CMake\CMake\bin\cmake.exe" --build feature_extraction/build --config Release -- /m
```
...
| 管线 | 耗时/对 | 吞吐 | 备注 |
|------|---------|-------|------|
| Python 全流程（ALIKED + 单个 LightGlue） | **187.13 ms** | **5.34 pairs/s** | `compare_py_cpp.py`, 12 pairs, 2 warmup, `max-edge=1024` |
| C++ TRT GPU（backbone FP32 + SDDH/LG FP16） | **73.30 ms** | **13.64 pairs/s** | `benchmark_cpp.py`, 12 pairs, 2 warmup, `max-edge=1024` |
...
|------|--------------|----------|--------------|----------|------------|--------|
| Python 全流程（ALIKED + 单个 LightGlue） | **44.81 ms / image** | **22.32 img/s** | **97.51 ms / pair** | **10.26 pair/s** | **187.13 ms / pair** | **5.34 pair/s** |
| C++ compare 模式（含 dump / 对齐开销） | **33.96 ms / image** | **29.45 img/s** | **40.61 ms / pair** | **24.62 pair/s** | **108.52 ms / pair** | **9.21 pair/s** |
| C++ benchmark 稳态（左图缓存，纯 GPU 路径） | **32.74 ms / image** | **30.55 img/s** | **40.58 ms / pair** | **24.64 pair/s** | **73.30 ms / pair** | **13.64 pair/s** |
