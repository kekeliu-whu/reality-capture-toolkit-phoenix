...
bool
PolyFisheyeCamera::Parameters::readFromYamlFile( const std::string& filename, const std::vector<std::string>& parameter_nodes )
{
    ...
    cv::FileNode camera_params = fs.root();
    for (const std::string& node_name : parameter_nodes) {
        camera_params = camera_params[node_name];
        if (camera_params.isNone()) {
            return false;
        }
    }

    std::string sModelType;
    camera_params["model_type"] >> sModelType;
    ...
    m_cameraName = camera_params["camera_name"].string();
    m_imageWidth  = static_cast<int>(camera_params["image_width"]);
    m_imageHeight = static_cast<int>(camera_params["image_height"]);

    cv::FileNode n = camera_params["projection_parameters"];
